...
message there, the bot **opens a thread** on that message and replies with an
**AI-generated response** (as the persona "Vibe Apply"). When a human posts
inside a thread under that channel, it replies again — and each thread is a
continuous conversation with memory. It ignores bots and itself, so it never
loops.

Replies are generated by your **local Claude Code CLI** in headless mode, so
they run under **your Claude subscription** (no separate API key or billing).
Tools and MCP are disabled for those calls, so untrusted Discord messages can't
make Claude run anything on your machine.
...
- **Claude Code CLI**, installed and logged in — the bot shells out to `claude`
  to write replies. Verify with `claude --version`, and make sure you've signed
  in (run `claude` once) so it uses your subscription. Docs:
...
| ------------------------------------------------ | ----------------------------------------------- |
| a message in the watched channel (human)         | opens a `Vibe Apply` thread and replies (AI)    |
| a message inside a thread under that channel     | replies again, remembering the thread so far    |
| a message from a bot, or the bot's own reply     | ignores it (no loops)                           |
| a message in any other channel                   | ignores it                                      |

Each thread maps to one Claude session, so follow-ups have context. The mapping
is in-memory, so after a restart older threads simply begin a fresh
conversation. Replies are capped to Discord's 2000-character limit, and at most
a few are generated at once.

> Note: the bot replies to **every** human message in the channel, so a busy
> channel will grow a lot of threads and use more of your Claude limits. That's
> by design — open an issue if you'd like a cooldown or one-thread-per-user
> option.
